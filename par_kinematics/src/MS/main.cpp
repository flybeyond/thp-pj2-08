/*
 * http://www.sensi.org/~svo/dbot/applet/ to C++
 */
...
// (look at pics above for explanation)
 ...
 const double rf = 62.0;    // base arm length
...
int main()
{
	 ...
	 * height: 600px
	 */
	//DeltaRobot deltaRobot(150.0, 310.0, 35.0, 20.0);
	DeltaRobot deltaRobot(62, 150, 73, 10); // hip, ankle, base, effector
	double x = 0;
	double y = 198.15;
	double z = 0;
    Point goal(x, y, z);
	...
	/* print statistics simulation model:
	 */
	std::cout << "Effector position: [" << goal.x << ", " << goal.y << ", " << goal.z << "]" << std::endl;
	std::cout << "Servo angles: " << Util::deg(deltaRobot.servo[0]) << ", " << Util::deg(deltaRobot.servo[1]) << ", " << Util::deg(deltaRobot.servo[2]) << std::endl;
	std::cout << "Ankle joints: " << Util::deg(deltaRobot.pangle[0]) << ", " << Util::deg(deltaRobot.pangle[1]) << ", " << Util::deg(deltaRobot.pangle[2]) << std::endl;
	std::cout << "Servo min/max/range: " << Util::deg(deltaRobot.servoMin) << ", " << Util::deg(deltaRobot.servoMax) << ", " << Util::deg(deltaRobot.servoMax - deltaRobot.servoMin) << std::endl;
	std::cout << "Hip: " << deltaRobot.getHip() << " Ankle: " << deltaRobot.getAnkle() << std::endl;
	std::cout << "Base: " << deltaRobot.getBase() << " Effector: " << deltaRobot.getEffector() << std::endl;

	/* print statistics hardware model:
	 */
	double th1, th2, th3;
	delta_calcInverse(x, y, z, th1, th2, th3);
	std::cout << "Servo angles: " << th1 << ", " << th2 << ", " << th3 << std::endl;
